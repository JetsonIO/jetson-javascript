...
Parse.prototype.addNode = function (node, type, matchValue) {
  ...
  nodeInstance.loc = this.pointer, nodeInstance.type = type;
...
Parse.prototype.checkNode = function nextNode(node, currentChar) {
  ...
  if ("whole" in node) {
  ...
  } else if ("tokenWhole" in node && this.currentChar() === node.tokenWhole) {
    return this.addNode(node, "whole", String.fromCharCode(node.tokenWhole));
  } else {
    if ("tokenStart" in node && this.currentChar() === node.tokenStart) {
    ...
    if ("tokenEnd" in node && this.currentChar() === node.tokenEnd) {
